...
export async function POST(request: Request) {
  try {
    ...
    if (!enforceHourlyLimit(user.id)) {
      return Response.json({ error: 'Too many unlock attempts. Try again later.' }, { status: 429 })
    }

    const lockoutMsg = checkLockout(user)
    if (lockoutMsg) {
      return Response.json({ error: lockoutMsg, requiresReauth: user.pinAttempts >= 20 }, { status: 403 })
    }
